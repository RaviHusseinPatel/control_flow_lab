...
public class SecretNumber {
    //    This is our Psuedo Code, used to guide anyone reading our code through our steps and processes (cmd/)
...
//    RULES 1.
    //        Always run the code to check if it works- less time finding issue
    //      Finish codes with semi colon
...
//        Enter output in brackets (in quotes)
//        println for line, print for nolnbrk
...
//        Now let's start our game. First we have to select a random number
//        But, to make the game better lets create a genuine random number, that changes each time, so even us as the game creator can play
//        'Random' is a variable


         Random generator1 = new Random();  //

//      As it is a number game, we use an int. Assign our class a title, here it is randomNumber
//        Entitle the integer secretNumber, as sensible name for our random secret number
//        This will be equal to our random class (generator)then .nextInt   nextInt returns a value between 0 and whatever number is between our brackets
//        Outside the brackets you can put +x  x is any number you want t start from added to 0

        int secretNumber = generator1.nextInt(1000) + 1;
